...
final class MainViewController: UIViewController, AVCaptureMetadataOutputObjectsDelegate {
    ...
    func metadataOutput(_ output: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        guard let obj = metadataObjects.first as? AVMetadataMachineReadableCodeObject, let raw = obj.stringValue, let data = raw.data(using: .utf8) else { return }
        do {
    ...
    private func saveConfig(_ cfg: MirrorConfig) {
        guard let ud = UserDefaults(suiteName: appGroupId), let data = try? JSONEncoder().encode(cfg) else { return }
